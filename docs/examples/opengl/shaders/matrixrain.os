...
void main() {
    ...
    // Matrix rain effect
    float rain = hash(grid + floor(u_time * 3.0));
    float rainIntensity = smoothstep(0.7, 1.0, rain);
    
    // Vertical streaming
    float stream = sin(uv.y * 20.0 + u_time * 10.0 + hash(vec2(grid.x, 0.0)) * 6.28318);
    stream = smoothstep(0.0, 1.0, stream);
    
    // Digital characters effect
    float character = step(0.5, hash(grid + floor(u_time * 5.0)));
    
    // Color
    vec3 greenMatrix = vec3(0.0, 1.0, 0.3);
    vec3 darkGreen = vec3(0.0, 0.3, 0.1);
    
    vec3 color = mix(vec3(0.0), darkGreen, stream * 0.3);
    color = mix(color, greenMatrix, rainIntensity * character * stream);
    
    // Add glow effect
    float glow = rainIntensity * character * stream;
    color += greenMatrix * glow * 0.5;
