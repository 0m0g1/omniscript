...
void main() {
    ...
    vec2 center = vec2(-0.7489, 0.149); // 🔁 change this to your target Mandelbrot location
    ...
    if (iterations < maxIterations) {
        // Rainbow coloring with time animation
        float hue = t * 6.0 + u_time * 0.5;
        float sat = 0.8 + sin(t * 10.0 + u_time) * 0.2;
        float val = 1.0 - t * 0.3;
        color = hsv2rgb(vec3(hue, sat, val));
    } else {
        // Inside set - dark with subtle color
        color = vec3(0.1, 0.05, 0.2) * (1.0 + sin(u_time * 2.0) * 0.3);
    }
