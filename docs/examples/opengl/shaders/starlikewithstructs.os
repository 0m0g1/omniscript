...
void main() {
    ...
    color += intensity * rainbowColor * 2.0;
...
// Performance configuration struct
...
// Fullscreen quad geometry struct
...
struct ShaderProgram {
    ...
    compileShader(source: char*, type: uint) => uint {
    ...
    create() => int {
        ...
        let vertexShader = this.compileShader(&vertexShaderSource, GL_VERTEX_SHADER);
        printf("Compiled vertex shader\n");
        if (vertexShader == 0) return 0;

        let fragmentShader = this.compileShader(&fragmentShaderSource, GL_FRAGMENT_SHADER);
        ...
        glad_glAttachShader(this.programId, vertexShader);
        glad_glAttachShader(this.programId, fragmentShader);
        glad_glLinkProgram(this.programId);

        let success: int;
        glad_glGetProgramiv(this.programId, GL_LINK_STATUS, &success);
        if (!success) {
            let infoLog: char[512];
            glad_glGetProgramInfoLog(this.programId, 512, nullptr, &infoLog);
            printf("Shader linking error:\n%s\n", &infoLog);
            glad_glDeleteProgram(this.programId);
            this.programId = 0;
        }
        ...
        if (this.programId != 0) {
            this.timeLocation = glad_glGetUniformLocation(this.programId, "u_time");
            this.resolutionLocation = glad_glGetUniformLocation(this.programId, "u_resolution");
            printf("Time location: %d, Resolution location: %d\n", this.timeLocation, this.resolutionLocation);
        }

        return this.programId != 0 ? 1 : 0;
    }
    ...
    setResolution(width: float, height: float) => void {
...
struct AppState {
    ...
    constructor() => void {
        ...
        this.performanceConfig = PerformanceConfig(120, 60);
    }
}

// Global application state
let g_appState: AppState;
...
function renderThreadFunction(param: void*) => uint {
    ...
    glfwMakeContextCurrent(g_appState.window);
    ...
    while (!g_appState.shouldExit) {
        ...
        glfwGetFramebufferSize(g_appState.window, &width, &height);
        ...
        shaderProgram.use();
        shaderProgram.setTime(currentTime);
        shaderProgram.setResolution(width as float, height as float);
        ...
        // Sleep based on performance config
        Sleep(g_appState.performanceConfig.getRenderSleepTime());
    }
...
function main() => void {
    ...
    // Create window
    g_appState.window = glfwCreateWindow(800, 600, "Threaded Star Field", nullptr, nullptr);
    ...
    let threadId: uint;
    g_appState.renderThread = CreateThread(nullptr, 0, renderThreadFunction, nullptr, 0, &threadId);
    if (g_appState.renderThread == nullptr) {
    ...
    while (!glfwWindowShouldClose(g_appState.window)) {
        glfwPollEvents();
        
        // Sleep based on performance config
        Sleep(g_appState.performanceConfig.getEventSleepTime());
    }

    printf("Main thread signaling render thread to exit\n");
    g_appState.shouldExit = 1;

    // Wait for render thread to finish
    printf("Waiting for render thread to finish\n");
    WaitForSingleObject(g_appState.renderThread, INFINITE);
    CloseHandle(g_appState.renderThread);
