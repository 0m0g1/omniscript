...
void main() {
    ...
    // Night sky gradient
    vec3 skyColor = mix(vec3(0.02, 0.05, 0.15), vec3(0.1, 0.2, 0.4), uv.y * 0.5 + 0.5);
    vec3 color = skyColor;
    
    // Aurora waves
    float wave1 = sin(uv.x * 3.0 + u_time * 1.2) * cos(uv.y * 1.5 + u_time * 0.8);
    float wave2 = sin(uv.x * 5.0 - u_time * 1.5) * cos(uv.y * 2.0 - u_time * 1.0);
    float wave3 = sin(uv.x * 2.0 + u_time * 0.9) * cos(uv.y * 3.0 + u_time * 1.3);
    
    // Combine waves
    float aurora = (wave1 + wave2 * 0.7 + wave3 * 0.5) * 0.5;
    
    // Aurora intensity based on position
    float intensity = smoothstep(-0.3, 0.3, aurora) * (1.0 - abs(uv.y * 0.8));
    
    // Aurora colors (green and purple dominant)
    vec3 auroraGreen = vec3(0.0, 1.0, 0.3);
    vec3 auroraPurple = vec3(0.8, 0.2, 1.0);
    vec3 auroraBlue = vec3(0.2, 0.6, 1.0);
    
    // Mix colors based on wave patterns
    vec3 auroraColor = mix(auroraGreen, auroraPurple, sin(aurora * 5.0 + u_time) * 0.5 + 0.5);
    auroraColor = mix(auroraColor, auroraBlue, sin(aurora * 3.0 - u_time * 0.7) * 0.3 + 0.3);
    
    // Add aurora to sky
    color += auroraColor * intensity * 0.8;
    
    // Add shimmer effect
    float shimmer = sin(uv.x * 15.0 + u_time * 4.0) * cos(uv.y * 10.0 + u_time * 3.0);
    color += auroraColor * intensity * shimmer * 0.2;
