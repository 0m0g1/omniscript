...
void main() {
    ...
    // Ocean waves
    float wave1 = sin(uv.x * 4.0 + u_time * 2.0) * 0.1;
    float wave2 = sin(uv.x * 8.0 - u_time * 1.5) * 0.05;
    float wave3 = sin(uv.x * 12.0 + u_time * 3.0) * 0.02;
    
    float totalWave = wave1 + wave2 + wave3;
    
    // Water surface
    float waterSurface = uv.y - totalWave;
    
    // Sky colors
    vec3 skyColor = mix(vec3(0.3, 0.6, 1.0), vec3(0.8, 0.9, 1.0), uv.y * 0.5 + 0.5);
    
    // Ocean colors
    vec3 oceanDeep = vec3(0.0, 0.2, 0.4);
    vec3 oceanShallow = vec3(0.0, 0.4, 0.6);
    vec3 oceanColor = mix(oceanDeep, oceanShallow, -waterSurface * 2.0);
    
    // Foam and reflection
    float foam = smoothstep(-0.02, 0.02, waterSurface);
    float reflection = smoothstep(-0.1, 0.0, waterSurface);
    ...
    if (waterSurface > 0.0) {
        ...
        // Add wave highlights
        float highlight = abs(sin(uv.x * 20.0 + u_time * 4.0)) * reflection;
    ...
    // Sun reflection
    float sunReflection = max(0.0, sin(uv.x * 2.0 + u_time) * reflection);
    color += vec3(1.0, 0.9, 0.6) * sunReflection * 0.3;
