...
void main() {
    ...
    // Liquid motion
    vec2 motion = vec2(
        sin(u_time * 0.8 + uv.y * 3.0) * 0.3,
        cos(u_time * 0.6 + uv.x * 4.0) * 0.2
    );
    
    vec2 distortedUV = uv + motion;
    
    // Metallic surface
    float surface = fbm(distortedUV * 4.0 + u_time * 0.5);
    float surface2 = fbm(distortedUV * 8.0 - u_time * 0.3);
    
    // Combine surfaces
    float metal = surface * 0.7 + surface2 * 0.3;
    
    // Metallic coloring
    vec3 baseColor = vec3(0.2, 0.3, 0.8);  // Blue metal
    vec3 highlightColor = vec3(0.9, 0.95, 1.0);  // Bright highlights
    vec3 darkColor = vec3(0.05, 0.08, 0.2);  // Dark reflections
    
    // Create metallic effect
    float highlight = smoothstep(0.6, 0.8, metal);
    float shadow = smoothstep(0.2, 0.4, metal);
    
    vec3 color = mix(darkColor, baseColor, shadow);
    color = mix(color, highlightColor, highlight);
    
    // Add reflective streaks
    float streaks = abs(sin(uv.x * 20.0 + u_time * 3.0 + metal * 10.0));
    streaks = smoothstep(0.8, 1.0, streaks);
    color += vec3(0.3, 0.4, 0.8) * streaks * 0.5;
