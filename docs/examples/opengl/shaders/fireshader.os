...
void main() {
    ...
    // Fire shape
    float fire = fbm(vec2(uv.x * 6.0, uv.y * 3.0 - u_time * 2.0));
    fire += fbm(vec2(uv.x * 12.0, uv.y * 6.0 - u_time * 4.0)) * 0.5;
    
    // Mask to create flame shape
    float mask = smoothstep(0.0, 0.3, uv.y) * smoothstep(1.0, 0.7, uv.y);
    mask *= smoothstep(0.0, 0.1, uv.x) * smoothstep(1.0, 0.9, uv.x);
    
    fire *= mask;
    
    // Fire colors
    vec3 fireColor = vec3(0.0);
    fireColor += vec3(1.0, 0.0, 0.0) * smoothstep(0.0, 0.3, fire);  // Red
    fireColor += vec3(1.0, 0.5, 0.0) * smoothstep(0.2, 0.5, fire);  // Orange
    fireColor += vec3(1.0, 1.0, 0.0) * smoothstep(0.4, 0.7, fire);  // Yellow
    fireColor += vec3(1.0, 1.0, 1.0) * smoothstep(0.6, 1.0, fire);  // White hot
    
    // Add flickering
    float flicker = sin(u_time * 10.0 + uv.x * 20.0) * 0.1 + 0.9;
    fireColor *= flicker;
