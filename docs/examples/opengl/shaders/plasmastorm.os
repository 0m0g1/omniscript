...
void main() {
    ...
    // Plasma layers
    float plasma1 = sin(uv.x * 3.0 + u_time * 2.0) * cos(uv.y * 2.0 + u_time * 1.5);
    float plasma2 = sin(uv.x * 5.0 - u_time * 1.8) * cos(uv.y * 4.0 - u_time * 2.2);
    float plasma3 = sin(length(uv) * 10.0 + u_time * 3.0);
    
    // Electric arcs
    float arc1 = abs(sin(uv.x * 15.0 + u_time * 4.0 + plasma1 * 5.0));
    float arc2 = abs(sin(uv.y * 12.0 + u_time * 3.5 + plasma2 * 4.0));
    
    // Combine plasma
    float plasma = (plasma1 + plasma2 + plasma3) * 0.33;
    
    // Create electric effect
    float electric = smoothstep(0.8, 1.0, arc1) + smoothstep(0.85, 1.0, arc2);
    
    // Color mapping
    color += hsv2rgb(vec3(plasma * 0.5 + u_time * 0.3, 0.8, 1.0)) * 0.6;
    color += hsv2rgb(vec3(0.7 + plasma * 0.3, 0.9, 1.0)) * electric * 2.0;
