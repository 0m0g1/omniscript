...
void main() {
    ...
    vec2 spherical = vec2(atan(ray.x, ray.z), asin(ray.y));
    
    // Scale and create star pattern
    vec2 starCoords = spherical * 20.0;
    float stars = 0.0;
    
    for (int i = 0; i < 3; i++) {
        float layer = pow(2.0, float(i));
        vec2 coord = starCoords * layer + u_time * 0.1 * layer;
        float n = noise(coord);
        stars += smoothstep(0.99, 1.0, n) / layer;
    }
    
    // Add some nebula-like background
    vec3 nebula = vec3(0.0);
    for (int i = 0; i < 4; i++) {
        float scale = pow(2.0, float(i));
        vec2 coord = starCoords * scale * 0.1 + u_time * 0.05;
        float n = noise(coord);
        nebula += vec3(n * 0.3, n * 0.2, n * 0.5) / scale;
    }
    
    vec3 color = stars * vec3(1.0, 0.9, 0.8) + nebula * 0.5;
