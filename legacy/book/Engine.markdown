...
## Purpose
The `Engine` class in the OmniScript++ (OS) compiler serves as the primary entry point for the compiler’s command-line interface and execution flow. It parses command-line arguments, configures the compilation process, and orchestrates the compilation of OmniScript++ source files (e.g., `starfield.os`, `types.os`) using the `Compiler`, `Lexer`, and `Parser` components. It supports Just-In-Time (JIT), Ahead-Of-Time (AOT), and hybrid compilation modes, handles target configuration, and provides utilities for reading source code, managing resources, and reporting errors. The `Engine` ensures robust argument parsing, target resolution, and execution statistics collection, making it the central hub for initiating compilation.

## Declarations
Below is the header file for `Engine`, with `<omniscript/omniscript_pch.h>` replaced by the necessary standard library includes.
...
### Explanation
- **Constants**:
  - `VERSION`: Defines the compiler version ("1.0.0").
  - `MAX_ARGS`: Limits command-line arguments to 1024.
  - `DEFAULT_THREAD_COUNT`: Default thread count for parallel compilation (0).
- **Enums**:
  - `ParseResult`: Defines outcomes of argument parsing (e.g., Success, InvalidArgument).
- **Structs**:
  - `ExecutionStats`: Tracks execution metrics (total time, parse time, compile time, memory usage, result).
  - `ArgumentParser`: Contains static maps (`argHandlers`, `archMap`, `osMap`, `gcMap`, `safetyMap`) for parsing command-line arguments and a `hash` function for string hashing.
- **Public Methods**:
  - `parseArguments`: Parses command-line arguments into a `Config` object.
  - `run`: Executes the compilation pipeline, from parsing to code generation.
  - `parseSizeString`: Converts size strings (e.g., "64MB") to bytes.
  - `parseTargetArch`, `parseTargetOS`, `parseTargetTriple`, `parseGCStrategy`, `parseSafetyLevel`: Parse specific configuration values.
  - `readSourceCode`: Reads source code from files or stdin.
  - `printUsage`, `listTargets`, `printVersion`: Output usage information, available targets, and version.
- **Private Methods**:
  - `handle*Args`: Handle specific argument groups (debug, target, optimization, runtime, security, linking).
  - `resolveTargetConfiguration`: Resolves auto-detected target architecture and OS.
  - `setDefaultOutputPath`: Sets default output path based on input file and format.
  - `printConfigDebugInfo`: Outputs configuration details in debug mode.
  - `getCurrentMemoryUsage`, `trackResourceUsage`: Track memory usage.
  - `formatError`, `handleCriticalError`: Format and handle errors.
- **Members**:
  - `currentStats_`: Thread-local storage for execution statistics.

## Definitions
Below is the implementation file for `Engine`, with necessary includes.
...
void Engine::printUsage() noexcept {
    ...
    console.log("  --emit-object            Emit object file");
    console.log("  --emit-bitcode           Emit bitcode");
    console.log("  --emit-wasm              Emit WebAssembly");
...
### Explanation
- **Argument Parsing**:
  - `parseArguments`: Processes command-line arguments, populating a `Config` object using `ArgumentParser::argHandlers`. Handles flags like `--main-source`, `--execute`, `--make`, and sets defaults (e.g., JIT mode, output path).
  - `ArgumentParser`: Maps arguments to handlers and provides lookup tables for architectures, OSes, GC strategies, and safety levels.
- **Execution**:
  - `run`: Orchestrates the compilation pipeline, reading source code, parsing with `Lexer` and `Parser`, and invoking `Compiler` for AOT/hybrid modes or `LLVMJITBackend` for JIT mode.
- **Utilities**:
  - `parseSizeString`: Converts size strings (e.g., "64MB") to bytes with suffix support.
  - `parseTargetArch`, `parseTargetOS`, `parseTargetTriple`, `parseGCStrategy`, `parseSafetyLevel`: Map strings to enum values, with fallbacks for invalid inputs.
  - `readSourceCode`: Reads source from files or stdin, handling large files efficiently.
  - `printUsage`, `listTargets`, `printVersion`: Provide user-facing information.
  - `resolveTargetConfiguration`: Resolves auto-detected targets using `TargetInfo`.
  - `setDefaultOutputPath`: Sets output path based on input file and format.
  - `printConfigDebugInfo`: Outputs configuration details in debug mode.
  - `getCurrentMemoryUsage`, `trackResourceUsage`: Track memory usage with platform-specific APIs.
  - `formatError`, `handleCriticalError`: Manage error reporting.
  - `handle*Args`: Specialized handlers for debug, target, optimization, runtime, security, and linking arguments.

## Usage in OS Compiler
The `Engine` class is the main entry point for running the OS compiler from the command line. Examples include:

- **AOT Compilation**:
  ```bash
  omniscript --make --target-arch x86_64 --target-os linux --output starfield starfield.os
  ```
  ```cpp
  int main(int argc, char* argv[]) {
      auto config = Engine::parseArguments(argc, argv);
      if (!config) {
          Engine::handleCriticalError(config.error());
      }
...
- **Debug Mode with WebAssembly**:
  ```bash
  omniscript --debug --emit-wasm starfield.os
  ```
  ```cpp
  auto config = Engine::parseArguments(argc, argv);
  if (config) {
...
These examples show how `Engine` processes command-line arguments and compiles scripts like `starfield.os` or `types.os`.

## Development Notes
The `Engine` class was designed to be the user-facing interface for the OS compiler, handling command-line input and orchestrating the compilation pipeline. Key design decisions include:
- **Argument Parsing**: Uses a map-based handler system for extensibility and maintainability.
- **Error Handling**: Employs `std::expected` and `error::globalErrorCollector` for robust error reporting.
- **Resource Tracking**: Collects execution statistics with `perf::ScopedTimer` and `perf::MemoryTracker`.
- **Modularity**: Delegates to `Lexer`, `Parser`, `Compiler`, and `LLVMJITBackend` for specific tasks.
Challenges included designing a flexible argument parsing system and ensuring robust file I/O for source reading. The commented-out code (e.g., parser error handling, parse tree printing) suggests incomplete features. The `Engine` was developed after `EngineConfigs` and `TargetInfo` to leverage their functionality and before `Compiler` to provide configuration.

## Dependencies
- **Standard Library**: Uses `<unordered_map>`, `<string>`, `<vector>`, `<span>`, `<expected>`, `<chrono>`, `<functional>`, `<iostream>`, `<filesystem>`, `<fstream>`, `<sstream>`, `<algorithm>`.
- **Platform-Specific**: `<sys/resource.h>`, `<malloc.h>` (Linux), `<windows.h>` (Windows), `<mach/mach.h>`, `<sys/resource.h>` (macOS) for memory usage tracking.
- **OS Components**:
  - `<omniscript/Core.h>`: For `console`, `DEBUG_LOG`, `error::globalErrorCollector`, `perf::ScopedTimer`, `perf::MemoryTracker`.
  - `<omniscript/TargetInfo.h>`: For target detection and triple parsing.
  - `<omniscript/EngineConfigs.h>`: For `Config` and related structs.
  - `<omniscript/Compiler.h>`: For `Compiler` in AOT/hybrid modes.
  - `<omniscript/Lexer.h>`, `<omniscript/Parser.h>`: For parsing source code.
  - `<omniscript/Backends/llvm/LLVMJITBackend.h>`: For JIT execution.

## Source Code
- Header: [https://github.com/0m0g1/omniscript/blob/main/include/omniscript/Engine.h](https://github.com/0m0g1/omniscript/blob/main/include/omniscript/Engine.h)
- Implementation: [https://github.com/0m0g1/omniscript/blob/main/src/Engine.cpp](https://github.com/0m0g1/omniscript/blob/main/src/Engine.cpp)

## Integration with Project
- **File Placement**:
  - Header: `include/omniscript/Engine.h`
  - Implementation: `src/Engine.cpp`
- **Build System**: The `premake5.lua` script includes `Engine.cpp` in the source file list and `Engine.h` in the include path. Requires dependencies (`Core.h`, `TargetInfo.h`, `EngineConfigs.h`, `Compiler.h`, `Lexer.h`, `Parser.h`, `LLVMJITBackend.h`) in the include path. Platform-specific headers are handled via conditional compilation.
